...
view.model.list = function (p) {
    ...
    trs_enter = trs.enter().append("tr").attr("class", "topics_row").attr("id", function(d){
        return "rowid_" + d.t
    });
    trs.exit().remove();
    ...
    divs = trs_enter.append("td").append("div").attr("class", function(d){
        return "spark " + d.t
    })//.classed("spark", true).classe;
