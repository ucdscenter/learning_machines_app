...
function renderFilterDocs(articles, dbn, qry){
    ...
    docCount.dimension(respd)
     ...
     $('.explore-docs-btn').click(function(e){
      let selected_docs = minSelect.dimension().top(30000)
      ...
      renderExploreDocs(selected_docs, dbn, qry, fromhistory)
     })
...
function renderExploreDocs(articles, dbn, qry){
  console.log(dbn)
  ...
  $('.filtered-count').text(articles.length)
  $('.total-filtered-count').text(articles.length)


  $('.filtered-count').css('color', updateEstimatedTime(articles.length, dbn, 'vis'))
